自動總結本專案的 git 變更、列出清單，並在用戶確認後 commit。
...
顯示完整摘要前，先用 Bash **平行**執行以下指令：

- `git config user.name` — 取得使用者名稱
- `git config user.email` — 取得使用者 Email
- `git branch --show-current` — 取得目前所在的 branch

在摘要最後顯示：

```
👤 目前 Git 帳號：<user.name> (<user.email>)
🌿 目前 Branch：<branch-name>
```

然後問用戶：

> 「以上是本次的變更摘要。確認要 commit 嗎？可直接回覆「確認」，或告訴我要修改 commit 訊息。」

**收到確認之前，不可執行任何 git 寫入操作。**

## 步驟 4 — 執行 commit（只在用戶確認後）

1. `git add` 所有相關變更的檔案（**不用** `git add -A`，逐一列出檔案以避免加入敏感檔）
2. 用 HEREDOC 格式執行 `git commit -m`，commit 訊息**只包含實際內容，不加任何 Co-Authored-By 或 AI 相關資訊**
3. 回報 commit 成功，並顯示最新的 commit hash

## 步驟 5 — 例外處理

- **沒有任何變更**：告知用戶「目前沒有未提交的變更，工作目錄是乾淨的。」
- **有 untracked 的敏感檔案**（`.env`、`*secret*`、`*credential*`）：警告用戶，不將其加入 commit
- **pre-commit hook 失敗**：**不使用 `--amend`**，修正問題後重新 stage，建立一個全新的 commit
- **Merge conflict 標記**（`<<<<<<`、`=======`、`>>>>>>>`）殘留在檔案中：中斷並警告用戶先解決衝突
