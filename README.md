...
4. **標籤尺寸：** 11mm × 13mm，上方功能名稱，下方小字快捷鍵，建議使用相片紙或名片紙。專案已附上可直接列印的 `Keycap_Labels_Print.pdf`，裁切後直接放入鍵帽即可。
...
以下為日後可考慮加入的功能方向，尚未實作。

### 1. 按鍵編輯桌面軟體

目前自訂按鍵需要修改 `keymap.h` 並重新燒錄韌體。未來可透過 CircuitPython 的 USB 隨身碟機制，讓 Macro Pad 插上電腦後直接以讀寫設定檔的方式更新按鍵配置，無需 IDE；規模更大時可考慮 [Vial](https://get.vial.today/) 相容層，提供現成的跨平台 GUI。

### 2. 軌跡球滑鼠控制

在外殼整合軌跡球，搭配 **PMW3360** 感測器（SPI 直連 RP2040）與 34mm 撞球，實現左手不離鍵盤的滑鼠控制。外殼需重新設計以容納球座，建議作為獨立的 V3.0 硬體版本規劃。

### 3. 小型顯示螢幕

加入 **SSD1306 OLED（128×64，I2C）** 顯示目前按鍵模式、Claude 連線狀態或最後執行的指令。RP2040 剩餘 GPIO 足夠支援；若需要彩色顯示可升級至 ST7789 TFT，但功耗與接線複雜度會提高。三項功能中投資報酬率最高，建議優先實作。
